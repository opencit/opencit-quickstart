...
if java_ready_report; then
  ...
  echo "export JAVA_CMD=$java" >> $CIT_ENV/cit-java
else
...
if [ -n "$CIT_USERNAME" ] && [ "$CIT_USERNAME" != "root" ] && [ -d /etc/authbind/byport ]; then
  touch /etc/authbind/byport/${JETTY_PORT:-80} /etc/authbind/byport/${JETTY_SECURE_PORT:-443}
  chmod 500 /etc/authbind/byport/${JETTY_PORT:-80} /etc/authbind/byport/${JETTY_SECURE_PORT:-443}
  chown $CIT_USERNAME /etc/authbind/byport/${JETTY_PORT:-80} /etc/authbind/byport/${JETTY_SECURE_PORT:-443}
fi
